...
- [x] `users.email` — NOT NULL (вставка без email кидает исключение)
- [x] `users.email` — UNIQUE (вставка дубликата кидает исключение)
- [x] `cart_items.quantity` — CHECK > 0 (вставка с quantity=0 кидает исключение)
