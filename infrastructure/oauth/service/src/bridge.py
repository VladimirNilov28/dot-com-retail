...
def handle_login(config: Config, login_challenge: str, cookie_header: Optional[str]) -> str:
    """Returns the URL the browser should be redirected to next."""
    login_request = hydra_client.get_login_request(config.hydra_admin_url, login_challenge)
    ...
    if login_request.get("skip"):
        ...
        session = kratos_client.whoami(config.kratos_public_url, cookie_header)
        if session is None:
    ...
    # the identity and threaded through Hydra's login `context`, which Hydra
    # carries forward to the matching consent request.
    identity = kratos_client.get_identity(config.kratos_admin_url, subject)
    role = (identity.get("metadata_admin") or {}).get("role")
    context = {"role": role} if role else None

    accepted = hydra_client.accept_login_request(
        config.hydra_admin_url, login_challenge, subject, context=context
    )
    return accepted["redirect_to"]


def handle_consent(config: Config, consent_challenge: str) -> str:
    ...
    consent_request = hydra_client.get_consent_request(config.hydra_admin_url, consent_challenge)
